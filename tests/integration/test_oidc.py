...
def test_oidc(browser, oidc_client, oidc_provider, oidc_scopes, person, user_account, user_password):
    oidc_session = SimpleNamespace(
        state=hex_string(16),
        nonce=hex_string(16),
    )

    auth_req = oidc_client.construct_AuthorizationRequest(request_args={
