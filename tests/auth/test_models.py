...
class test_user_account_manager:
    ...
    def test_authenticate(self):
        ...
        assert authenticate(request=None, username='firstlast', password='wrong') is None
        assert authenticate(request=None, username='dunno', password='abcdefgh') is None
        assert authenticate(request=None, username='firstlast', password='abcdefgh') == expected_user


class test_service_account:
    def test_authenticate(self):
        expected_service_account = factories.ServiceAccountFactory(
            token='goodtoken',
        )

        assert authenticate(request=None, service_token='badtoken') is None
        assert authenticate(request=None, token='goodtoken') is None
        assert authenticate(request=None, service_token='goodtoken') == expected_service_account
    ...
    def test_authenticate_from_request(self, request_factory):
        ...
        assert authenticate(
            request=request_factory.get('/', HTTP_AUTHORIZATION='Service-Token badtoken'),
        ) is None
        assert authenticate(
            request=request_factory.get('/', HTTP_AUTHORIZATION='Token goodtoken'),
        ) is None
        assert authenticate(
            request=request_factory.get('/', HTTP_AUTHORIZATION='Service-Token goodtoken'),
